...
int main(int argc, char* argv[]) {
    ...
    pRenderDevice->GetEngineFactory()->CreateDefaultShaderSourceStreamFactory("Assets/Shaders/", &pShaderSourceFactory);
    ...
    Diligent::RefCntAutoPtr<Diligent::ITexture> pTextureOutput;
    {
...

        {
            ...
            pRenderDevice->CreateTexture(desc, nullptr, &pTextureInput);
        }
    ...
    while (!glfwWindowShouldClose(pWindow.get())) {
        ...
        Diligent::Viewport viewPort = {0.0f, 0.0f, static_cast<float>(pTextureOutput->GetDesc().Width), static_cast<float>(pTextureOutput->GetDesc().Height) };
        Diligent::Rect scissor = { 0, 0, static_cast<int32_t>(pTextureOutput->GetDesc().Width), static_cast<int32_t>(pTextureOutput->GetDesc().Height) };
        ...
        pDeviceContext->SetViewports(1, &viewPort, pSwapChain->GetDesc().Width, pSwapChain->GetDesc().Height);
        pDeviceContext->SetScissorRects(1, &scissor, pSwapChain->GetDesc().Width, pSwapChain->GetDesc().Height);
